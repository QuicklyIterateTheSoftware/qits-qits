...
| `artifacts.sweep` | artifacts | `POST /artifacts/api/gc/sweep {pins}`; SKIPPED("dry run") when dryRun | artifacts.plan, pins.deployments, pins.ci, pins.dependencies, pins.images |
...
each member the peer's answer verbatim, each absent when unread (fail-closed at qits-artifacts).
`pins.dependencies` is qits-platform-maintenance's manifest keep-set (internal maven/npm/docker
versions repos' mains reference); `pins.images` is qits-configuration's configured container images
(workspace / editor / project-agent / refinement). The two `artifacts.usage.*` steps read the
registry store's own bytes, which live in postgres and are invisible to the docker usage read —
the 2026-09-04 storage incident was 50 GB no receipt showed.
