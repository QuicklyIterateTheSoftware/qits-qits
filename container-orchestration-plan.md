...
| 10 | USER DIRECTIVE: full clean rebootstrap keeping config volumes, green start-to-finish | **DONE 2026-08-12: attempt 11 exit 0, 69 ok + 1 warm skip = 70/70, ZERO warns, 17 qits-pd containers healthy, edge 200. LIVE RESTART PROOF on the fresh platform: workload created via machine-token REST, `docker restart` of the deployed orchestrator, container Id/StartedAt byte-identical after, row re-answered RUNNING, logs answered, DELETE clean.** Attempt history (each failure fixed at source, whole run repeated) — attempt 7 — attempt 7 (2026-08-12) reached 68/70 ok in 23m24s, the FULL 16-deployable train built and deployed through orchestrator-spawned CI steps; 2 warns being fixed: (a) qits-artifacts webui clone 404 — its .gitmodules still names the renamed qits-platform-spa-artifacts; (b) qits-containers post-receive — the adduser/chown/su pattern is IMPOSSIBLE under --cap-drop=ALL (no CAP_CHOWN/SETUID even for root; eventstream's 7b50cad shipped the same never-exercised pattern). Fix in flight: steps START non-root via a new step-level `user:` plumbed yml→qits-ci→ContainerSpec `--user`; maven-base gains a passwd-backed build user + pre-owned /workspace; both pipelines drop the su dance; artifacts .gitmodules → ../qits-spa-artifacts.git. Earlier failure classes this campaign fixed at source: lost release stamps (restamp + tags + memory), seed lib order both lists + sentinel, native reflection (SpecFingerprint family + ErrorBody, coverage-walk test), Central throttling (persistent qits-maven-cache, cache-class in unwrap), main-head release scripts vs old-tag trees (presence guard). |
